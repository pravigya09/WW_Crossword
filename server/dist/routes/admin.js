...
adminRouter.get('/clues', requireAdmin, (_req, res) => {
    const clues = db.prepare('SELECT * FROM clue_sets WHERE puzzle_id IS NULL ORDER BY created_at ASC').all();
    res.json(clues);
});
// POST /api/admin/clues
adminRouter.post('/clues', requireAdmin, (req, res) => {
    const { clue, answer, hint } = req.body;
    const cleaned = answer.toUpperCase().replace(/[^A-Z]/g, '');
    if (!cleaned)
        return res.status(400).json({ error: 'Answer must contain letters only' });
    if (!clue.trim())
        return res.status(400).json({ error: 'Clue text required' });
    const id = uuidv4();
...
adminRouter.put('/clues/:id', requireAdmin, (req, res) => {
    const { clue, answer, hint } = req.body;
    const cleaned = answer.toUpperCase().replace(/[^A-Z]/g, '');
    if (!cleaned)
        return res.status(400).json({ error: 'Answer must contain letters only' });
...
adminRouter.delete('/clues/:id', requireAdmin, (req, res) => {
...
adminRouter.post('/generate', requireAdmin, (_req, res) => {
    const clues = db.prepare('SELECT * FROM clue_sets WHERE puzzle_id IS NULL').all();
    if (clues.length < 2) {
...
adminRouter.post('/puzzles', requireAdmin, (req, res) => {
    const { title, grid, startAt, endAt, timeLimitSeconds, hintMode } = req.body;
    const clues = db.prepare('SELECT * FROM clue_sets WHERE puzzle_id IS NULL').all();
    const id = uuidv4();
    db.prepare(`
...
adminRouter.get('/puzzles', requireAdmin, (_req, res) => {
...
adminRouter.get('/puzzles/:id/stats', requireAdmin, (req, res) => {
...
adminRouter.delete('/puzzles/:id', requireAdmin, (req, res) => {
    db.prepare('DELETE FROM puzzles WHERE id=?').run(req.params.id);
    res.json({ ok: true });
});
// PUT /api/admin/puzzles/:id — update puzzle settings
adminRouter.put('/puzzles/:id', requireAdmin, (req, res) => {
    const { title, startAt, endAt, timeLimitSeconds, hintMode } = req.body;
    db.prepare(`
