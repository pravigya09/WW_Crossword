...
function makeGrid(size: number): Grid {
  return Array.from({ length: size }, () => Array(size).fill(null) as (string | null)[]);
}
...
function canPlace(grid: Grid, word: string, row: number, col: number, dir: 'across' | 'down'): boolean {
  ...
  if (row - dr >= 0 && col - dc >= 0 && grid[row - dr][col - dc] !== null) return false;
  // Cell after word end must be empty
  if (endRow + dr < size && endCol + dc < size && grid[endRow + dr][endCol + dc] !== null) return false;
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (cell !== null) {
      ...
      const perpChecks: [number, number][] = dir === 'across'
        ? [[r - 1, c], [r + 1, c]]
        : [[r, c - 1], [r, c + 1]];
      for (const [pr, pc] of perpChecks) {
        if (pr >= 0 && pr < size && pc >= 0 && pc < size && grid[pr][pc] !== null) return false;
      }
    }
  }
...
function countIntersections(grid: Grid, word: string, row: number, col: number, dir: 'across' | 'down'): number {
  const dr = dir === 'down' ? 1 : 0;
  const dc = dir === 'across' ? 1 : 0;
  let n = 0;
  for (let i = 0; i < word.length; i++) {
    if (grid[row + dr * i][col + dc * i] === word[i]) n++;
  }
  return n;
}

function placeWord(grid: Grid, word: string, row: number, col: number, dir: 'across' | 'down') {
  const dr = dir === 'down' ? 1 : 0;
  const dc = dir === 'across' ? 1 : 0;
  for (let i = 0; i < word.length; i++) {
...
function findBestPlacement(grid: Grid, word: string): { row: number; col: number; dir: 'across' | 'down'; intersections: number } | null {
  const size = grid.length;
...
function getBoundingBox(grid: Grid): { minR: number; maxR: number; minC: number; maxC: number } {
  const size = grid.length;
...
export function generateCrossword(entries: WordEntry[]): CrosswordGrid {
  ...
  for (const size of [15, 17, 19, 21]) {
    ...
    if (placed.size === sorted.length) break;
  }
  ...
  const trimmed: Grid = Array.from({ length: newSize }, () => Array(newSize).fill(null));
  ...
  const numbering: number[][] = Array.from({ length: newSize }, () => Array(newSize).fill(0));
  let cellNum = 1;
  ...
  const placedWords: PlacedWord[] = adjustedPlacements.map(p => ({
    word: p.word,
    clue: p.clue,
    hint: p.hint,
    row: p.row,
  ...
  const cells: (GridCell | null)[][] = Array.from({ length: newSize }, () =>
    Array(newSize).fill(null) as (GridCell | null)[]
  );
  ...
  for (const pw of placedWords) {
    ...
    for (let i = 0; i < pw.word.length; i++) {
      const r = pw.row + dr * i;
      const c = pw.col + dc * i;
      if (!cells[r][c]) cells[r][c] = { letter: pw.word[i], wordIds: [] };
      if (!cells[r][c]!.wordIds.includes(pw.number)) cells[r][c]!.wordIds.push(pw.number);
    }
  }
